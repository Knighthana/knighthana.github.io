...
继上次"应该是最后一次更新这篇文章了"之后，这里还是要再来一笔，不过这次之后应该真的不需要再往这个页面写大事记了——维护日志都放在了[博客维护专题](/categories/Blog-Maintenance/)里。

这次改动的主旨是：**仓库只存原料，构建与部署全部交给 GitHub Actions**。

- 部署方式从"本地 `hexo generate` 后提交 `docs/` 产物"改为"push 即自动构建发布"（Actions 构建 → Pages artifact 部署）。
- `docs/` 目录退役，不再进版本库（`.gitignore`），成为纯构建输出。
- 图片迁入 `builder/source/img/`，通过 Git LFS 管理（`.gitattributes`），构建时由 CI 拉取真实文件。
- Hexo 工程（`builder/`）完整入仓，本地无需再维护独立的 `blog_builder`。
- 本地预览：`cd builder && npx hexo server`；发布：直接 `git push`。

详细的踩坑记录与改造过程见[博客构建链路改造记录](/BlogMaintenance/博客构建链路改造记录.html)。
